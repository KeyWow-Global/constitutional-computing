...
All numerical fixture values, including amounts, replica counts, and version integers, have this designation:
...
Each expected file contains an `expected` object with only the relevant observation dimension: `decision` for decision cases, `execution` for C05, `confirmation` for C11, or `demo_call` for C12. `REJECTED` is only the fixture-specific expected result of the C12 direct demo-call test; it is not a general evaluation outcome, execution status, or confirmation status. These fields do not introduce a generalized state model; type annotations in `models.py` list only values returned by the harness functions and are not enforced at runtime.
...
> The reproduction writer stores a caller-supplied `evaluation_log` value as canonical JSON without interpreting it; the harness defines no event-name or event-order model.
